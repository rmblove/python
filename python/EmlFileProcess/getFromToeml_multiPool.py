...
class emailProcess:
    ...
    def process(self,  amount_to_process = 0):
        ...
        if lenth==0: return
        simplebar_handle = Timelinebar(lenth)
        simplebar_handle.setinfo(self.eml_path)
        timebar_process = Process(target=simplebar_handle.run, args=(self.q,))
        timebar_process.start()
        simplesql_handle = simpleSqlite()
        simpsql_process = Process(target=simplesql_handle.run, args=(self.task_q,))
